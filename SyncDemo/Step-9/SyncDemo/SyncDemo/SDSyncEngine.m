...
- (void)deleteObjectsOnServer
{
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        for (NSManagedObject *objectToDelete in objectsToDelete) {
            ...
            //
            NSMutableURLRequest *request = [[SDAFParseAPIClient sharedClient] DELETERequestForClass:className forObjectWithId:[objectToDelete valueForKey:@"objectId"]];
...
- (void)processJSONDataRecordsForDeletion
{
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        if ([JSONRecords count] > 0) {
            ...
            //
            NSArray *storedRecords = [self managedObjectsForClass:className sortedByKey:@"objectId" usingArrayOfIds:[JSONRecords valueForKey:@"objectId"] inArrayOfIds:NO];
